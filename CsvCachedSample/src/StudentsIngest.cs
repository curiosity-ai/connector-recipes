...
// Same shape as CsvSample's StudentsIngest, exposing the row model, the
// schema registration, and the per-row ingestion logic. The HashCache wiring
// lives in Program.cs so this file stays focused on graph mapping.
public static class StudentsIngest
{
    // Bump when the mapping below changes shape (new property, new edge, new
    // composite key). Every cached hash from older versions becomes a miss
    // and the affected rows get re-ingested on the next run.
    public const int ConnectorVersion = 1;
